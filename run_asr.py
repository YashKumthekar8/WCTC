...
epoch = 50
wctc = True
output_dir = './output/librispeech.asr.True.0.2'
...
class Orthography:
    ...
    def create_processor(self, model_args: ModelArguments) -> Wav2Vec2Processor:
        if model_name_or_path == 'facebook/wav2vec2-large':
...
def main():
    ...
    # dataset = datasets.load_dataset('timit_asr')
    # train_dataset = dataset['train']
    # test_dataset = dataset['test']

    # train_dataset = datasets.load_dataset(
    #     dataset_name, data_dir='Timit\TRAIN', split=train_split_name
    # )
    
    # val_dataset = datasets.load_dataset(
    #     dataset_name, data_dir='Timit\TEST', split=validation_split_name
    # )

    train_dataset = load_dataset('librispeech_asr', 'clean', split='train.100', streaming=True)
    val_dataset = load_dataset('librispeech_asr', 'clean', split='test', streaming=True)
    ...
    def prepare_example(example, mask_ratio=0.2):  # TODO(elgeish) make use of multiprocessing?
    ...
    # train_dataset = train_dataset.map(prepare_example, fn_kwargs={"mask_ratio": mask_ratio}, keep_in_memory=True, load_from_cache_file=False, remove_columns=[data_args.speech_file_column])
    ...
    logger.warning(f"Updated {len(text_updates)} transcript(s) using '{orthography}' orthography rules.")
