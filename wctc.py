...
def wctc_loss(log_probs : torch.Tensor, targets : torch.Tensor, input_lengths : torch.Tensor, target_lengths : torch.Tensor, 
    ...
    # As flattened targets are 1D tensor it converts into 2D array with row as sequence
    ...
    # The second tensor is a boolean tensor that checks if each element in the tensor _t_a_r_g_e_t_s_ starting from the third column is not equal to the element in the same row but two columns before it. This is done using the != operator.
    ...
    # Gathers elements from log_probs where indices are provided by _t_a_r_g_e_t_s_
    log_probs_ = log_probs.gather(-1, _t_a_r_g_e_t_s_.expand(input_time_size, -1, -1))

    # Initializes log_alpha to zeros with size specified by input_time_size, batch_size, zero_padding + _t_a_r_g_e_t_s_.shape[-1]
    ...
    # Selects blank-th element from logprobs and feed it to log_alpha
    log_alpha[0, :, zero_padding + 0] = log_probs[0, :, blank]

    # Selects second element from each array in _targets_ from the Bth array 
    log_alpha[0, :, zero_padding + 1] = log_probs[0, B, _t_a_r_g_e_t_s_[:, 1]]

    # Calculating previous time steps values 
